...
export default function DateRangePicker({
  ...
  const handleApply = () => {
    let next = { ...temp };
    if (minDate && next.from && isBefore(next.from, minDate)) next.from = minDate;
  ...
  const displayLabel = React.useMemo(() => {
    const r = applied;
    if (r?.from && r?.to) return `${format(r.from, formatString)} → ${format(r.to, formatString)}`;
  ...
  return (
    <div className={cn("w-full", className)}>
      ...
      >
        <PopoverTrigger asChild>
          ...
          >
            <span className="inline-flex items-center gap-2 truncate">
              ...
              <span className={cn("truncate", !applied?.from && "text-muted-foreground")}>
                {displayLabel}
              </span>
            </span>
        ...
        <PopoverContent
          align="end"
          className="w-[650px] max-w-[95vw] p-0 rounded-2xl shadow-xl border"
        >
          <div className="flex flex-col w-full overflow-x-auto">
            {/* Header */}
            <div className="px-4 pt-4 pb-2 text-sm font-medium text-muted-foreground">
              {temp?.from && temp?.to
                ? `${format(temp.from, formatString)} → ${format(temp.to, formatString)}`
            ...
            <div className="flex flex-col sm:flex-row gap-4 px-4 pb-4">
              ...
              {/* Calendar */}
              <div className="grow rounded-xl border overflow-x-auto min-w-[300px] sm:min-w-[600px]">
                <Calendar
                  mode="range"
                  ...
                  className="p-2"
                  classNames={{
                    months: "flex flex-row gap-4", // force side-by-side
                    month: "space-y-2",
                    ...
                    head_cell: "text-[11px] font-semibold text-muted-foreground",
                    row: "grid grid-cols-7",
                    cell: "h-9 w-9 p-0 text-center align-middle",
                    day: "h-9 w-9 flex items-center justify-center rounded-md hover:bg-muted focus-visible:outline-none",
            ...
            <div className="border-t px-4 py-3 flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <Button type="button" variant="ghost" size="sm" onClick={() => setTemp({ from: today, to: today })}>
                  Today
                </Button>
                {allowClear && (
              ...
              <div className="flex items-center gap-2">
                ...
                <Button type="button" onClick={handleApply} className="px-6" disabled={!temp?.from}>
                  Apply
                </Button>
              </div>
            </div>
